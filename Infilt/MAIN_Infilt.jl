...
module mainInfilt
	...
	function MAIN_INFILT(N_SoilSelect, Tinfilt, ∑Infilt, ∑Psd, N_Infilt, infilt, hydro)
		...
		# COMPUTING INFILTRATION FROM PHYSICAL HYDRAULIC PARAMETERS
			if option.θΨ ≠ "No" && (option.infiltration.OptimizeRun == "Run" ||  option.infiltration.OptimizeRun == "RunOpt") && option.infiltration.Model=="Simplified" && option.infiltration.SeIni_Range 
			...
			for iSoil=1:N_SoilSelect
				...
				# looping
					for iT = 1:param.infilt.Npoint_Infilt
						 ∑Infilt_Best_HydroObs[iSoil,iT] = best.BEST(iSoil, option.infiltration.Dimension, Sorptivity, Tinfilt_Best_HydroObs[iSoil,iT], infilt.θ_Ini[iSoil], hydro, infilt)
					end  # for iT=N_\Delta	
