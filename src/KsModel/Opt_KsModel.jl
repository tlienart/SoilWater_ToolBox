...
module optKsModel
	...
	#		FUNCTION : OF_KSMODEL
	# ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
		function OF_KSMODEL(hydro, ipLayer, KₛModel, KₛModel⍰, ksmodelτ, N_iZ, optim, optimKsmodel, X; Flag_IsTopsoil=false, Flag_RockFragment=false, IsTopsoil=[], RockFragment=[])
			...
			# NOT AS GOOD
				...
				# Of_Ks = stats.RMSE(log1p.(3600 * hydro.Ks[1:N_iZ]) , log1p.(3600 * KₛModel[1:N_iZ]); Power=2.0)
				# Of_Ks = stats.RMSE(log1p.(3600 * hydro.Ks[1:N_iZ]) , log1p.(3600 * KₛModel[1:N_iZ]); Power=4.0)
				# Of_Ks = stats.RMSE(log1p.(3600 * hydro.Ks[1:N_iZ]) , log1p.(3600 * KₛModel[1:N_iZ]); Power=6.0)
			# BADD
