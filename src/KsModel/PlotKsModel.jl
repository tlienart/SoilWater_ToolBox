		...
		import ..cst
			...
			function KsModel_3D(hydro, N_iZ, path)
				...
				Z1 = Float64.(hydro.θs[1:N_iZ] .- hydro.θr[1:N_iZ])
				Z2 = Float64.(hydro.σ[1:N_iZ])
				Z3 = Float64.(cst.MmS_2_MmH.*hydro.Ks[1:N_iZ])

				surface!(Axis1, Z1, Z2, Z3; shading=true, colormap = :deep, axis = (show_axis = true,))

				Axis2 = Axis(Fig[2,1])
				Z1b = Float64.(hydro.θsMacMat[1:N_iZ] .- hydro.θr[1:N_iZ])
				surface!(Axis2, Z1b, Z2, Z3; shading=true, colormap = :deep, axis = (show_axis = true,))

				Axis3 = Axis(Fig[3,1])
				Z1c = Float64.(hydro.θsMacMat[1:N_iZ] .- hydro.θr[1:N_iZ])
